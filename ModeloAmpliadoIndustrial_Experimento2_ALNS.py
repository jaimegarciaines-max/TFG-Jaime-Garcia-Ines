...
CARPETA_RESULTADOS = "resultados_experimento_2_alns"
    ...
    CONFIGURACIONES.append(
        {
            ...
            # Experimento 2: ALNS activado para mejorar la solución factible de Gurobi.
...
def crear_resumen_y_graficos_excel():
    ...
    - Graficos: gráficos principales del Experimento 2 y box plots de mejora.
    ...
    for row in range(2, ws_res.max_row + 1):
        ...
        alns_ejecutado = ws_res.cell(row=row, column=col["alns_ejecutado"]).value
        mejora_obj_pct = ws_res.cell(row=row, column=col["mejora_porcentual_objetivo_alns"]).value
        mejora_obj_abs = ws_res.cell(row=row, column=col["mejora_objetivo_alns"]).value
        mejora_coste_pct = ws_res.cell(row=row, column=col["mejora_porcentual_coste_setup_alns"]).value
        mejora_coste_abs = ws_res.cell(row=row, column=col["mejora_coste_setup_alns"]).value
        tiempo_alns = ws_res.cell(row=row, column=col["tiempo_alns"]).value
        iteraciones_alns = ws_res.cell(row=row, column=col["iteraciones_alns"]).value
        ...
        datos[n_orders]["alns_ejecutado"].append(alns_ejecutado)
        datos[n_orders]["mejora_obj_pct"].append(mejora_obj_pct)
        datos[n_orders]["mejora_obj_abs"].append(mejora_obj_abs)
        datos[n_orders]["mejora_coste_pct"].append(mejora_coste_pct)
        datos[n_orders]["mejora_coste_abs"].append(mejora_coste_abs)
        datos[n_orders]["tiempo_alns"].append(tiempo_alns)
        datos[n_orders]["iteraciones_alns"].append(iteraciones_alns)
    ...
    ws_graf["A1"] = "Gráficos del experimento 2: mejora de Gurobi mediante ALNS"
    ws_graf["A1"].font = Font(bold=True, size=14)

    max_row = ws_sum.max_row
    cats = Reference(ws_sum, min_col=1, min_row=2, max_row=max_row)

    # 1) % de instancias mejoradas por ALNS.
    chart_mejoradas = LineChart()
    chart_mejoradas.title = "% de instancias mejoradas por ALNS"
    chart_mejoradas.y_axis.title = "% instancias mejoradas"
    chart_mejoradas.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=22, min_row=1, max_row=max_row)
    chart_mejoradas.add_data(data, titles_from_data=True)
    chart_mejoradas.set_categories(cats)
    chart_mejoradas.height = 8
    chart_mejoradas.width = 18
    ws_graf.add_chart(chart_mejoradas, "A3")

    # 2) Mejora media objetivo (%).
    chart_obj = LineChart()
    chart_obj.title = "Mejora media del objetivo por ALNS"
    chart_obj.y_axis.title = "Mejora media objetivo (%)"
    chart_obj.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=23, min_row=1, max_row=max_row)
    chart_obj.add_data(data, titles_from_data=True)
    chart_obj.set_categories(cats)
    chart_obj.height = 8
    chart_obj.width = 18
    ws_graf.add_chart(chart_obj, "J3")

    # 3) Mejora media coste setup (%).
    chart_coste = LineChart()
    chart_coste.title = "Mejora media del coste setup por ALNS"
    chart_coste.y_axis.title = "Mejora media coste setup (%)"
    chart_coste.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=28, min_row=1, max_row=max_row)
    chart_coste.add_data(data, titles_from_data=True)
    chart_coste.set_categories(cats)
    chart_coste.height = 8
    chart_coste.width = 18
    ws_graf.add_chart(chart_coste, "A22")

    # 4) Tiempo medio ALNS.
    chart_tiempo_alns = LineChart()
    chart_tiempo_alns.title = "Tiempo medio ALNS por tamaño"
    chart_tiempo_alns.y_axis.title = "Tiempo medio ALNS (s)"
    chart_tiempo_alns.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=33, min_row=1, max_row=max_row)
    chart_tiempo_alns.add_data(data, titles_from_data=True)
    chart_tiempo_alns.set_categories(cats)
    chart_tiempo_alns.height = 8
    chart_tiempo_alns.width = 18
    ws_graf.add_chart(chart_tiempo_alns, "J22")

    # 5) Iteraciones medias ALNS.
    chart_iter_alns = LineChart()
    chart_iter_alns.title = "Iteraciones medias ALNS por tamaño"
    chart_iter_alns.y_axis.title = "Iteraciones medias ALNS"
    chart_iter_alns.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=35, min_row=1, max_row=max_row)
    chart_iter_alns.add_data(data, titles_from_data=True)
    chart_iter_alns.set_categories(cats)
    chart_iter_alns.height = 8
    chart_iter_alns.width = 18
    ws_graf.add_chart(chart_iter_alns, "A41")
    ...
    if plt is not None and ExcelImage is not None and datos:
        try:
            ...
            mejora_obj_series = [
                _valores_numericos(datos[n]["mejora_obj_pct"])
                for n in sorted(datos)
            ]
            mejora_coste_series = [
                _valores_numericos(datos[n]["mejora_coste_pct"])
                for n in sorted(datos)
            ]

            # Box plot mejora objetivo.
            plt.figure(figsize=(12, 5))
            plt.boxplot(mejora_obj_series, labels=labels, showfliers=True)
            plt.xlabel("Número de orders")
            plt.ylabel("Mejora objetivo (%)")
            plt.title("Distribución de la mejora porcentual del objetivo por tamaño")
            plt.xticks(rotation=45)
            plt.tight_layout()
            ruta_box_obj = os.path.join(CARPETA_RESULTADOS, "boxplot_mejora_objetivo_alns.png")
            plt.savefig(ruta_box_obj, dpi=160)
            plt.close()

            img_obj = ExcelImage(ruta_box_obj)
            img_obj.width = 720
            img_obj.height = 300
            ws_graf.add_image(img_obj, "J41")

            # Box plot mejora coste setup.
            plt.figure(figsize=(12, 5))
            plt.boxplot(mejora_coste_series, labels=labels, showfliers=True)
            plt.xlabel("Número de orders")
            plt.ylabel("Mejora coste setup (%)")
            plt.title("Distribución de la mejora porcentual del coste setup por tamaño")
            plt.xticks(rotation=45)
            plt.tight_layout()
            ruta_box_coste = os.path.join(CARPETA_RESULTADOS, "boxplot_mejora_coste_setup_alns.png")
            plt.savefig(ruta_box_coste, dpi=160)
            plt.close()

            img_coste = ExcelImage(ruta_box_coste)
            img_coste.width = 720
            img_coste.height = 300
            ws_graf.add_image(img_coste, "A60")
