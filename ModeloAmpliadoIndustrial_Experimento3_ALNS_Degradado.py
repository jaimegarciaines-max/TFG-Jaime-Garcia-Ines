...
CARPETA_RESULTADOS = "resultados_experimento_3_alns_degradado"
...
# Parámetros específicos del Experimento 3.
# La solución de Gurobi se degrada antes de ejecutar ALNS.
PORCENTAJE_ORDERS_SUSTITUIDAS = 0.20
MAX_INTENTOS_DEGRADACION = 1000
MAX_INTENTOS_LINEA_DEGRADACION = 100
MAX_INTENTOS_REEMPLAZO_LINEA = 100

# ----------------------------------------------------------------
# CONFIGURACIONES DEL EXPERIMENTO 3
# ----------------------------------------------------------------
# Objetivo: analizar si el ALNS tiene capacidad reparadora cuando parte de
# una solución factible degradada artificialmente.
    ...
    CONFIGURACIONES.append(
        {
            ...
            # Gurobi se mantiene igual que en los experimentos anteriores.
            "MIN_TIME": 120,
            "MAX_TIME": 120,
            "TARGET_GAP": 0,

            # ALNS parte de la solución degradada.
            "ALNS_MAX_ITER": 500000,
            "ALNS_MAX_TIME": 300,
            "ALNS_PORCENTAJE_REMOVAL": 0.10,

            # Degradación controlada.
            "PORCENTAJE_ORDERS_SUSTITUIDAS": PORCENTAJE_ORDERS_SUSTITUIDAS,
            "MAX_INTENTOS_DEGRADACION": MAX_INTENTOS_DEGRADACION,
        }
    )
...
def generar_instancia(instancia_id, indice_config, indice_local, config):
    ...
    instancia = {
        ...
        "porcentaje_orders_sustituidas": config.get("PORCENTAJE_ORDERS_SUSTITUIDAS", PORCENTAJE_ORDERS_SUSTITUIDAS),
        "max_intentos_degradacion": config.get("MAX_INTENTOS_DEGRADACION", MAX_INTENTOS_DEGRADACION),
        "use_seed": USE_SEED,
...
def guardar_input_generado(instancia):
    ...
    with open(ruta_input(instancia_id), "w", encoding="utf-8") as f:
        ...
        f.write(f"# PORCENTAJE_ORDERS_SUSTITUIDAS {instancia['porcentaje_orders_sustituidas']}\n")
        f.write(f"# MAX_INTENTOS_DEGRADACION {instancia['max_intentos_degradacion']}\n\n")
...
# ============================================================
# 4B. DEGRADACIÓN CONTROLADA DE SOLUCIONES
# ============================================================

def copiar_solucion(solucion):
    """Devuelve una copia independiente de una solución."""
    return {l: sec.copy() for l, sec in solucion.items()}


def calcular_diferencias_orders(sol_base, sol_comparada):
    """
    Compara dos soluciones por conjunto de orders asignadas.
    Devuelve cuántas orders salen y cuántas entran en la solución comparada.
    """
    base = set(pedidos_asignados(sol_base))
    comparada = set(pedidos_asignados(sol_comparada))

    sacadas = base - comparada
    introducidas = comparada - base

    porcentaje_cambiadas = (
        100 * len(introducidas) / len(base)
        if len(base) > 0 else 0
    )

    return {
        "orders_sacadas": len(sacadas),
        "orders_introducidas": len(introducidas),
        "porcentaje_orders_cambiadas": porcentaje_cambiadas,
    }


def degradar_solucion_gurobi(solucion_gurobi, instancia):
    """
    Genera una solución factible degradada a partir de la solución de Gurobi.

    La degradación combina dos mecanismos:
    1. Barajado del orden de las orders dentro de cada línea abierta.
    2. Sustitución de un porcentaje de orders asignadas por orders pendientes,
       manteniendo la factibilidad de cargas y compatibilidades.

    La sustitución se hace por línea para evitar cerrar líneas por debajo del
    mínimo de carga. Si no se logra una sustitución factible tras varios
    intentos, se usa una degradación basada solo en barajado de secuencias.
    """
    orders = instancia["orders"]
    lines = instancia["lines"]
    weight = instancia["weight"]
    ...
    penalizacion = instancia["penalizacion_linea_cerrada"]
    porcentaje = instancia["porcentaje_orders_sustituidas"]
    max_intentos = instancia["max_intentos_degradacion"]

    if instancia["seed_instancia"] != "NO_SEED":
        rng = random.Random(instancia["seed_instancia"] + 202603)
    else:
        rng = random.Random()

    obj_gurobi_calc = objetivo_total(
        solucion_gurobi,
        orders,
        lines,
        coste,
    ...
    mejor_barajada = copiar_solucion(solucion_gurobi)
    for l in lines:
        rng.shuffle(mejor_barajada[l])

    # Si el barajado por azar mejora, se reintenta unas veces para obtener una degradación real.
    for _ in range(50):
        candidata_barajada = copiar_solucion(solucion_gurobi)
        for l in lines:
            rng.shuffle(candidata_barajada[l])
        if objetivo_total(candidata_barajada, orders, lines, coste, coste_inicio, penalizacion) >= obj_gurobi_calc:
            mejor_barajada = candidata_barajada
            break

    for _ in range(max_intentos):
        sol = copiar_solucion(solucion_gurobi)
        asignadas_originales = set(pedidos_asignados(solucion_gurobi))
        disponibles = set(pedidos_pendientes(solucion_gurobi, orders))
        total_sacadas = 0
        total_introducidas = 0
        intento_valido = True

        for l in lines:
            sec_original = solucion_gurobi[l]
            if len(sec_original) == 0:
                continue

            k = max(1, int(round(len(sec_original) * porcentaje)))
            k = min(k, len(sec_original))

            exito_linea = False

            for _linea in range(MAX_INTENTOS_LINEA_DEGRADACION):
                quitar = rng.sample(sec_original, k)
                quitar_set = set(quitar)
                sec_sin_quitar = [i for i in sec_original if i not in quitar_set]
                carga_base = sum(weight[i] for i in sec_sin_quitar)

                candidatos = [
                    p for p in disponibles
                    if compatible[(p, l)] == 1
                    and p not in asignadas_originales
                ]

                if len(candidatos) < k:
                    continue

                for _rep in range(MAX_INTENTOS_REEMPLAZO_LINEA):
                    meter = rng.sample(candidatos, k)
                    carga_nueva = carga_base + sum(weight[p] for p in meter)

                    if min_load - 1e-9 <= carga_nueva <= max_load + 1e-9:
                        nueva_sec = sec_sin_quitar + meter
                        rng.shuffle(nueva_sec)
                        sol[l] = nueva_sec
                        ...
                        asignadas_originales.difference_update(quitar)
                        asignadas_originales.update(meter)

                        total_sacadas += len(quitar)
                        total_introducidas += len(meter)
                        exito_linea = True
                        break
        ...
        if not es_factible(sol, orders, lines, weight, min_load, max_load, compatible):
            continue

        obj_degradado = objetivo_total(sol, orders, lines, coste, coste_inicio, penalizacion)

        # Se exige que la solución degradada no sea mejor que la solución de Gurobi.
        if obj_degradado + 1e-9 < obj_gurobi_calc:
            continue

        stats = {
            "degradacion_ejecutada": True,
            "degradacion_factible_completa": True,
            "orders_sustituidas_degradacion": total_introducidas,
            "porcentaje_orders_sustituidas_degradacion": (
                100 * total_introducidas / len(pedidos_asignados(solucion_gurobi))
                if len(pedidos_asignados(solucion_gurobi)) > 0 else 0
            ),
            "obj_degradado": obj_degradado,
            "coste_setup_degradado": coste_solucion(sol, coste, coste_inicio),
            "degradacion_pct": (
                100 * (obj_degradado - obj_gurobi_calc) / obj_gurobi_calc
                if abs(obj_gurobi_calc) > 1e-9 else 0
            ),
        }
        return sol, stats

    # Fallback: si no se consigue sustitución factible, se usa solo el barajado.
    obj_degradado = objetivo_total(
        mejor_barajada,
        orders,
        lines,
        coste,
    ...
    stats = {
        "degradacion_ejecutada": True,
        "degradacion_factible_completa": False,
        "orders_sustituidas_degradacion": 0,
        "porcentaje_orders_sustituidas_degradacion": 0,
        "obj_degradado": obj_degradado,
        "coste_setup_degradado": coste_solucion(mejor_barajada, coste, coste_inicio),
        "degradacion_pct": (
            100 * (obj_degradado - obj_gurobi_calc) / obj_gurobi_calc
            if abs(obj_gurobi_calc) > 1e-9 else 0
        ),
...
def alns(solucion_inicial, orders, lines, coste, coste_inicio, weight,
    ...
    # Contadores de diagnóstico del ALNS.
    # Sirven para analizar si el algoritmo no mejora porque no genera
    # soluciones factibles, porque no acepta candidatas o porque las
    # candidatas factibles no son mejores que la solución inicial.
    candidatas_generadas = 0
    candidatas_factibles = 0
    candidatas_infactibles = 0
    ...
    estadisticas = {
        ...
        "candidatas_generadas": candidatas_generadas,
        "candidatas_factibles": candidatas_factibles,
        "candidatas_infactibles": candidatas_infactibles,
        "aceptadas": aceptadas,
        "rechazadas": rechazadas,
        "mejoras_globales": mejoras_globales,
        "mejoras_actuales": mejoras_actuales,
        "aceptadas_peores": aceptadas_peores,
        "porcentaje_candidatas_factibles": (
            100 * candidatas_factibles / candidatas_generadas
            if candidatas_generadas > 0 else 0
        ),
        "porcentaje_candidatas_aceptadas": (
            100 * aceptadas / candidatas_factibles
            if candidatas_factibles > 0 else 0
        )
    }
...
def resolver_instancia(instancia, total_instancias):
    ...
    Después, degrada la solución de Gurobi y ejecuta ALNS desde esa solución
    degradada para evaluar su capacidad reparadora.
    """
    instancia_id = instancia["instancia_id"]
    orders = instancia["orders"]
    lines = instancia["lines"]
    ...
    resultado = {
        ...
        "solucion_degradada": None,
        "solucion_alns": None,
        "alns_ejecutado": False,
        "stats_alns": None,
        "stats_degradacion": None,
        "mip_gap": None,
    ...
    if m.SolCount > 0:
        ...
        # Experimento 3: ALNS se ejecuta desde una solución degradada, también si Gurobi fue óptimo.
        if diagnostico_gurobi["secuencias_validas"]:
            solucion_degradada, stats_degradacion = degradar_solucion_gurobi(
                solucion_gurobi,
                instancia
            )

            resultado["solucion_degradada"] = solucion_degradada
            resultado["stats_degradacion"] = stats_degradacion
            resultado["obj_degradado"] = stats_degradacion["obj_degradado"]
...
def guardar_output_instancia(instancia, resultado):
    ...
    with open(ruta_output(instancia_id), "w", encoding="utf-8") as f:
        ...
        if resultado["solucion_gurobi"] is not None:
            ...
            if resultado.get("solucion_degradada") is not None:
                stats_deg = resultado.get("stats_degradacion") or {}
                escribir("==================================================", f)
                escribir("SOLUCIÓN DEGRADADA", f)
                escribir("==================================================", f)
                escribir(f"Degradación factible completa: {stats_deg.get('degradacion_factible_completa')}", f)
                escribir(f"Orders sustituidas en degradación: {stats_deg.get('orders_sustituidas_degradacion')}", f)
                escribir(f"Porcentaje orders sustituidas: {stats_deg.get('porcentaje_orders_sustituidas_degradacion')} %", f)
                escribir(f"Objetivo degradado: {stats_deg.get('obj_degradado')}", f)
                escribir(f"Coste setup degradado: {stats_deg.get('coste_setup_degradado')}", f)
                escribir(f"Degradación objetivo respecto a Gurobi: {stats_deg.get('degradacion_pct')} %", f)
                escribir("", f)
                escribir_resumen_solucion("Degradada", resultado["solucion_degradada"], instancia, f)
            ...
            if resultado["alns_ejecutado"]:
                ...
                escribir(f"Objetivo inicial tomado de solución degradada: {stats['objetivo_inicial']}", f)
                escribir(f"Mejor objetivo encontrado por ALNS: {stats['mejor_objetivo']}", f)
                escribir(f"Coste setup inicial degradado: {stats['coste_setup_inicial']}", f)
                escribir(f"Coste setup mejor ALNS: {stats['coste_setup_mejor']}", f)
                escribir(f"Toneladas iniciales degradadas: {stats['toneladas_iniciales']}", f)
                escribir(f"Toneladas mejor ALNS: {stats['toneladas_mejor']}", f)
                escribir(f"Líneas abiertas inicial degradada: {stats['lineas_abiertas_inicial']}", f)
                escribir(f"Líneas abiertas mejor ALNS: {stats['lineas_abiertas_mejor']}", f)
                escribir("", f)
                escribir("Diagnóstico interno del ALNS:", f)
                escribir(f"Candidatas generadas: {stats.get('candidatas_generadas')}", f)
                escribir(f"Candidatas factibles: {stats.get('candidatas_factibles')}", f)
                escribir(f"Candidatas infactibles: {stats.get('candidatas_infactibles')}", f)
                escribir(f"Porcentaje de candidatas factibles: {stats.get('porcentaje_candidatas_factibles')} %", f)
                escribir(f"Candidatas aceptadas: {stats.get('aceptadas')}", f)
                escribir(f"Candidatas rechazadas: {stats.get('rechazadas')}", f)
                escribir(f"Porcentaje de candidatas aceptadas sobre factibles: {stats.get('porcentaje_candidatas_aceptadas')} %", f)
                escribir(f"Mejoras globales: {stats.get('mejoras_globales')}", f)
                escribir(f"Mejoras sobre solución actual: {stats.get('mejoras_actuales')}", f)
                escribir(f"Aceptadas peores: {stats.get('aceptadas_peores')}", f)
                ...
                if resultado.get("solucion_degradada") is not None:
                    obj_gurobi = resultado.get("obj_gurobi")
                    obj_degradado = resultado.get("obj_degradado")
                    obj_alns = stats["mejor_objetivo"]
                    if obj_gurobi is not None and obj_degradado is not None and abs(obj_degradado - obj_gurobi) > 1e-9:
                        recuperacion = 100 * (obj_degradado - obj_alns) / (obj_degradado - obj_gurobi)
                        escribir(f"Recuperación respecto a Gurobi: {recuperacion} %", f)
                    dif_orders = calcular_diferencias_orders(resultado["solucion_degradada"], resultado["solucion_alns"])
                    escribir(f"Orders sacadas ALNS vs degradada: {dif_orders['orders_sacadas']}", f)
                    escribir(f"Orders introducidas ALNS vs degradada: {dif_orders['orders_introducidas']}", f)
                    escribir(f"Porcentaje orders cambiadas ALNS vs degradada: {dif_orders['porcentaje_orders_cambiadas']} %", f)
...
COLUMNAS_EXCEL = [
    ...
    "porcentaje_orders_sustituidas_param",
    "use_seed",
    "seed_base",
    "seed_instancia",
    ...
    "degradacion_ejecutada",
    "degradacion_factible_completa",
    "obj_degradado",
    "coste_setup_degradado",
    "lineas_abiertas_degradado",
    "lineas_cerradas_degradado",
    "toneladas_asignadas_degradado",
    "toneladas_pendientes_degradado",
    "porcentaje_toneladas_degradado",
    "orders_asignadas_degradado",
    "orders_pendientes_degradado",
    "orders_sustituidas_degradacion",
    "porcentaje_orders_sustituidas_degradacion",
    "degradacion_objetivo_pct",
    "alns_ejecutado",
    "obj_alns",
    "tiempo_alns",
    "iteraciones_alns",
    "candidatas_generadas_alns",
    "candidatas_factibles_alns",
    "candidatas_infactibles_alns",
    "porcentaje_candidatas_factibles_alns",
    "candidatas_aceptadas_alns",
    "candidatas_rechazadas_alns",
    "porcentaje_candidatas_aceptadas_alns",
    "mejoras_globales_alns",
    "mejoras_actuales_alns",
    "aceptadas_peores_alns",
    ...
    "mejora_alns_sobre_degradado_abs",
    "mejora_alns_sobre_degradado_pct",
    "recuperacion_vs_gurobi_pct",
    "orders_sacadas_alns_vs_degradada",
    "orders_introducidas_alns_vs_degradada",
    "porcentaje_orders_cambiadas_alns_vs_degradada",
    "resultado_final",
    "input_file",
    "output_file"
]
...
def construir_fila_excel(instancia, resultado):
    ...
    fila["porcentaje_orders_sustituidas_param"] = 100 * instancia["porcentaje_orders_sustituidas"]
    ...
    fila["degradacion_ejecutada"] = resultado.get("solucion_degradada") is not None
    ...
    if resultado.get("solucion_degradada") is not None:
        sol = resultado["solucion_degradada"]
        stats_deg = resultado.get("stats_degradacion") or {}
        toneladas = toneladas_asignadas(sol, weight)
        fila["degradacion_factible_completa"] = stats_deg.get("degradacion_factible_completa")
        fila["obj_degradado"] = stats_deg.get("obj_degradado")
        fila["coste_setup_degradado"] = stats_deg.get("coste_setup_degradado")
        fila["lineas_abiertas_degradado"] = numero_lineas_abiertas(sol)
        fila["lineas_cerradas_degradado"] = lineas_cerradas(sol, lines)
        fila["toneladas_asignadas_degradado"] = toneladas
        fila["toneladas_pendientes_degradado"] = toneladas_totales - toneladas
        fila["porcentaje_toneladas_degradado"] = 100 * toneladas / toneladas_totales if toneladas_totales > 0 else 0
        fila["orders_asignadas_degradado"] = len(pedidos_asignados(sol))
        fila["orders_pendientes_degradado"] = len(pedidos_pendientes(sol, orders))
        fila["orders_sustituidas_degradacion"] = stats_deg.get("orders_sustituidas_degradacion")
        fila["porcentaje_orders_sustituidas_degradacion"] = stats_deg.get("porcentaje_orders_sustituidas_degradacion")
        fila["degradacion_objetivo_pct"] = stats_deg.get("degradacion_pct")
    ...
    if resultado["solucion_alns"] is not None:
        ...
        fila["candidatas_generadas_alns"] = stats.get("candidatas_generadas")
        fila["candidatas_factibles_alns"] = stats.get("candidatas_factibles")
        fila["candidatas_infactibles_alns"] = stats.get("candidatas_infactibles")
        fila["porcentaje_candidatas_factibles_alns"] = stats.get("porcentaje_candidatas_factibles")
        fila["candidatas_aceptadas_alns"] = stats.get("aceptadas")
        fila["candidatas_rechazadas_alns"] = stats.get("rechazadas")
        fila["porcentaje_candidatas_aceptadas_alns"] = stats.get("porcentaje_candidatas_aceptadas")
        fila["mejoras_globales_alns"] = stats.get("mejoras_globales")
        fila["mejoras_actuales_alns"] = stats.get("mejoras_actuales")
        fila["aceptadas_peores_alns"] = stats.get("aceptadas_peores")
        ...
        obj_ini = stats["objetivo_inicial"]
        obj_mejor = stats["mejor_objetivo"]
        coste_ini = stats["coste_setup_inicial"]
        coste_mejor = stats["coste_setup_mejor"]
        ...
        fila["mejora_alns_sobre_degradado_abs"] = obj_ini - obj_mejor
        fila["mejora_alns_sobre_degradado_pct"] = 100 * (obj_ini - obj_mejor) / obj_ini if abs(obj_ini) > 1e-9 else 0

        obj_gurobi = resultado.get("obj_gurobi")
        obj_degradado = resultado.get("obj_degradado")
        if obj_gurobi is not None and obj_degradado is not None and abs(obj_degradado - obj_gurobi) > 1e-9:
            fila["recuperacion_vs_gurobi_pct"] = 100 * (obj_degradado - obj_mejor) / (obj_degradado - obj_gurobi)

        if resultado.get("solucion_degradada") is not None:
            dif = calcular_diferencias_orders(resultado["solucion_degradada"], sol)
            fila["orders_sacadas_alns_vs_degradada"] = dif["orders_sacadas"]
            fila["orders_introducidas_alns_vs_degradada"] = dif["orders_introducidas"]
            fila["porcentaje_orders_cambiadas_alns_vs_degradada"] = dif["porcentaje_orders_cambiadas"]
...
# ============================================================
# 10. RESUMEN Y GRÁFICOS DEL EXPERIMENTO 2
# ============================================================
...
def crear_resumen_y_graficos_excel():
    ...
    - Graficos: gráficos principales del Experimento 3.
    ...
    required = [
        "num_orders", "estado_gurobi", "gap_gurobi", "tiempo_gurobi",
        "alns_ejecutado", "mejora_porcentual_objetivo_alns",
        "mejora_objetivo_alns", "mejora_porcentual_coste_setup_alns",
        "mejora_coste_setup_alns", "tiempo_alns", "iteraciones_alns",
        "degradacion_objetivo_pct", "mejora_alns_sobre_degradado_pct",
        "recuperacion_vs_gurobi_pct", "orders_sacadas_alns_vs_degradada",
        "orders_introducidas_alns_vs_degradada",
        "porcentaje_orders_cambiadas_alns_vs_degradada",
        "degradacion_factible_completa", "candidatas_generadas_alns",
        "candidatas_factibles_alns", "porcentaje_candidatas_factibles_alns"
    ]
    ...
    for row in range(2, ws_res.max_row + 1):
        ...
        datos.setdefault(n_orders, {
            ...
            "degradacion_pct": [],
            "degradacion_factible_completa": [],
            "mejora_sobre_degradado_pct": [],
            "recuperacion_vs_gurobi_pct": [],
            "orders_sacadas": [],
            "orders_introducidas": [],
            "porcentaje_orders_cambiadas": [],
            "candidatas_generadas": [],
            "candidatas_factibles": [],
            "porcentaje_candidatas_factibles": [],
        })

        def v(nombre):
            return ws_res.cell(row=row, column=col[nombre]).value

        datos[n_orders]["estados"].append(v("estado_gurobi"))
        datos[n_orders]["gaps"].append(v("gap_gurobi"))
        datos[n_orders]["tiempos"].append(v("tiempo_gurobi"))
        datos[n_orders]["alns_ejecutado"].append(v("alns_ejecutado"))
        datos[n_orders]["mejora_obj_pct"].append(v("mejora_porcentual_objetivo_alns"))
        datos[n_orders]["mejora_obj_abs"].append(v("mejora_objetivo_alns"))
        datos[n_orders]["mejora_coste_pct"].append(v("mejora_porcentual_coste_setup_alns"))
        datos[n_orders]["mejora_coste_abs"].append(v("mejora_coste_setup_alns"))
        datos[n_orders]["tiempo_alns"].append(v("tiempo_alns"))
        datos[n_orders]["iteraciones_alns"].append(v("iteraciones_alns"))
        datos[n_orders]["degradacion_pct"].append(v("degradacion_objetivo_pct"))
        datos[n_orders]["degradacion_factible_completa"].append(v("degradacion_factible_completa"))
        datos[n_orders]["mejora_sobre_degradado_pct"].append(v("mejora_alns_sobre_degradado_pct"))
        datos[n_orders]["recuperacion_vs_gurobi_pct"].append(v("recuperacion_vs_gurobi_pct"))
        datos[n_orders]["orders_sacadas"].append(v("orders_sacadas_alns_vs_degradada"))
        datos[n_orders]["orders_introducidas"].append(v("orders_introducidas_alns_vs_degradada"))
        datos[n_orders]["porcentaje_orders_cambiadas"].append(v("porcentaje_orders_cambiadas_alns_vs_degradada"))
        datos[n_orders]["candidatas_generadas"].append(v("candidatas_generadas_alns"))
        datos[n_orders]["candidatas_factibles"].append(v("candidatas_factibles_alns"))
        datos[n_orders]["porcentaje_candidatas_factibles"].append(v("porcentaje_candidatas_factibles_alns"))
    ...
    resumen_headers = [
        ...
        "porcentaje_instancias_mejoradas_alns",
        "mejora_media_objetivo_pct",
        "mejora_mediana_objetivo_pct",
        "mejora_maxima_objetivo_pct",
        "mejora_media_objetivo_abs",
        "mejora_mediana_objetivo_abs",
        "mejora_media_coste_setup_pct",
        "mejora_mediana_coste_setup_pct",
        "mejora_maxima_coste_setup_pct",
        "mejora_media_coste_setup_abs",
        "mejora_mediana_coste_setup_abs",
        "tiempo_medio_alns",
        "tiempo_mediana_alns",
        "iteraciones_medias_alns",
        "iteraciones_mediana_alns",
        "degradacion_media_objetivo_pct",
        "degradacion_mediana_objetivo_pct",
        "porcentaje_degradaciones_factibles_completas",
        "mejora_media_sobre_degradado_pct",
        "mejora_mediana_sobre_degradado_pct",
        "mejora_maxima_sobre_degradado_pct",
        "recuperacion_media_vs_gurobi_pct",
        "recuperacion_mediana_vs_gurobi_pct",
        "recuperacion_maxima_vs_gurobi_pct",
        "orders_sacadas_media_alns_vs_degradada",
        "orders_introducidas_media_alns_vs_degradada",
        "porcentaje_orders_cambiadas_medio_alns_vs_degradada",
        "candidatas_generadas_medias_alns",
        "candidatas_factibles_medias_alns",
        "porcentaje_candidatas_factibles_medio_alns",
    ]
    ...
    for n_orders in sorted(datos):
        ...
        mejora_obj_pct = _valores_numericos(datos[n_orders]["mejora_obj_pct"])
        mejora_obj_abs = _valores_numericos(datos[n_orders]["mejora_obj_abs"])
        mejora_coste_pct = _valores_numericos(datos[n_orders]["mejora_coste_pct"])
        mejora_coste_abs = _valores_numericos(datos[n_orders]["mejora_coste_abs"])
        tiempo_alns = _valores_numericos(datos[n_orders]["tiempo_alns"])
        iteraciones_alns = _valores_numericos(datos[n_orders]["iteraciones_alns"])
        degradacion_pct = _valores_numericos(datos[n_orders]["degradacion_pct"])
        mejora_sobre_degradado_pct = _valores_numericos(datos[n_orders]["mejora_sobre_degradado_pct"])
        recuperacion_vs_gurobi_pct = _valores_numericos(datos[n_orders]["recuperacion_vs_gurobi_pct"])
        orders_sacadas = _valores_numericos(datos[n_orders]["orders_sacadas"])
        orders_introducidas = _valores_numericos(datos[n_orders]["orders_introducidas"])
        porcentaje_orders_cambiadas = _valores_numericos(datos[n_orders]["porcentaje_orders_cambiadas"])
        candidatas_generadas = _valores_numericos(datos[n_orders]["candidatas_generadas"])
        candidatas_factibles = _valores_numericos(datos[n_orders]["candidatas_factibles"])
        porcentaje_candidatas_factibles = _valores_numericos(datos[n_orders]["porcentaje_candidatas_factibles"])
        ...
        num_alns = len(tiempo_alns)
        num_mejoradas = sum(1 for v in mejora_obj_abs if isinstance(v, (int, float)) and v > 1e-9)
        num_deg_completa = sum(1 for v in datos[n_orders]["degradacion_factible_completa"] if v is True)
        ...
        ws_sum.append([
            ...
            _media(mejora_obj_pct),
            _mediana(mejora_obj_pct),
            max(mejora_obj_pct) if mejora_obj_pct else None,
            _media(mejora_obj_abs),
            _mediana(mejora_obj_abs),
            _media(mejora_coste_pct),
            _mediana(mejora_coste_pct),
            max(mejora_coste_pct) if mejora_coste_pct else None,
            _media(mejora_coste_abs),
            _mediana(mejora_coste_abs),
            _media(tiempo_alns),
            _mediana(tiempo_alns),
            _media(iteraciones_alns),
            _mediana(iteraciones_alns),
            _media(degradacion_pct),
            _mediana(degradacion_pct),
            _porcentaje(num_deg_completa, total),
            _media(mejora_sobre_degradado_pct),
            _mediana(mejora_sobre_degradado_pct),
            max(mejora_sobre_degradado_pct) if mejora_sobre_degradado_pct else None,
            _media(recuperacion_vs_gurobi_pct),
            _mediana(recuperacion_vs_gurobi_pct),
            max(recuperacion_vs_gurobi_pct) if recuperacion_vs_gurobi_pct else None,
            _media(orders_sacadas),
            _media(orders_introducidas),
            _media(porcentaje_orders_cambiadas),
            _media(candidatas_generadas),
            _media(candidatas_factibles),
            _media(porcentaje_candidatas_factibles),
        ])
    ...
    for col_cells in ws_sum.columns:
        ...
        for cell in col_cells:
            if cell.value is not None:
                max_len = max(max_len, len(str(cell.value)))
        ws_sum.column_dimensions[col_letter].width = min(max_len + 2, 45)

    ws_graf = wb.create_sheet("Graficos")
    ws_graf["A1"] = "Gráficos del experimento 3: capacidad reparadora del ALNS"
    ws_graf["A1"].font = Font(bold=True, size=14)

    max_row = ws_sum.max_row
    cats = Reference(ws_sum, min_col=1, min_row=2, max_row=max_row)

    # Columnas en Resumen_por_tamano:
    # 37 degradación media, 40 mejora media sobre degradado, 43 recuperación media.
    chart_deg = LineChart()
    chart_deg.title = "Degradación media de la solución inicial"
    chart_deg.y_axis.title = "Degradación media (%)"
    chart_deg.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=37, min_row=1, max_row=max_row)
    chart_deg.add_data(data, titles_from_data=True)
    chart_deg.set_categories(cats)
    chart_deg.height = 8
    chart_deg.width = 18
    ws_graf.add_chart(chart_deg, "A3")

    chart_mejora = LineChart()
    chart_mejora.title = "Mejora media ALNS sobre solución degradada"
    chart_mejora.y_axis.title = "Mejora media (%)"
    chart_mejora.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=40, min_row=1, max_row=max_row)
    ...
    ws_graf.add_chart(chart_mejora, "J3")

    chart_rec = LineChart()
    chart_rec.title = "Recuperación media respecto a Gurobi"
    chart_rec.y_axis.title = "Recuperación media (%)"
    chart_rec.x_axis.title = "Número de orders"
    data = Reference(ws_sum, min_col=43, min_row=1, max_row=max_row)
    chart_rec.add_data(data, titles_from_data=True)
    chart_rec.set_categories(cats)
    chart_rec.height = 8
    chart_rec.width = 18
    ws_graf.add_chart(chart_rec, "A22")

    if plt is not None and ExcelImage is not None and datos:
        try:
            labels = [str(n) for n in sorted(datos)]
            recuperacion_series = [
                _valores_numericos(datos[n]["recuperacion_vs_gurobi_pct"])
                for n in sorted(datos)
            ]

            plt.figure(figsize=(12, 5))
            plt.boxplot(recuperacion_series, labels=labels, showfliers=True)
            plt.xlabel("Número de orders")
            plt.ylabel("Recuperación respecto a Gurobi (%)")
            plt.title("Distribución de la recuperación respecto a Gurobi por tamaño")
            plt.xticks(rotation=45)
            plt.tight_layout()
            ruta_box_rec = os.path.join(CARPETA_RESULTADOS, "boxplot_recuperacion_vs_gurobi.png")
            plt.savefig(ruta_box_rec, dpi=160)
            plt.close()

            img_rec = ExcelImage(ruta_box_rec)
            img_rec.width = 720
            img_rec.height = 300
            ws_graf.add_image(img_rec, "J22")
        except Exception as exc:
            ws_graf["J22"] = f"No se pudo generar el box plot automáticamente: {exc}"
    else:
        ws_graf["J22"] = "Box plot no generado: matplotlib u openpyxl Image no disponibles."
...
def main():
    ...
    if PRINT_PROGRESS:
        print("\n==================================================")
        print("RESUMEN Y GRÁFICOS DEL EXPERIMENTO 2 GENERADOS")
        print(f"Excel: {EXCEL_RESUMEN}")
        print("==================================================\n")
